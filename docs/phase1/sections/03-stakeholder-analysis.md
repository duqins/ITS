...
| Resource requests | Requester, department representative | Category, purpose, specifications, quantity, preferred condition, urgency, location, and required date | Department representative where the request commits the department. |
| Search and filtering | Requester, procurement officer | Category, condition, department, location, quantity, availability, asset age, value, and life-cycle status | No approval required. |
| AI matching and ranking | Requester, department representative | Meaning, category, functional purpose, technical compatibility, quantity, condition, location, urgency/date, transfer/repair cost, sustainability benefit, score, explanation, and confidence | User chooses a match; AI does not approve it. |
...
| Notifications and audit trail | All authorised users, administrators | Workflow events, recipients, timestamps, actor, decision, reason, and before/after values | Workflow owner defines alerts; audit records are not editable by ordinary users. |
